...
| F-03-3-007 | INFO | `picked` のブロック幅が `chunk_size` (実効値) を流用しており、代表目標行列の確保幅と1回の solve に畳む列数という別の軸が隠れて結合している。round4 (F-03-4-005) で送り理由を訂正: 当初の送り理由「`surrogate_capacities` 切り出しと同時にやると追加コストがほぼ無いため」は誤りだった (その切り出しは round3 でこの diff 自身が既に実施済みで、切り出しの機会に分離しなかった)。改めて 3b へ送る理由は、分離には「ブロック確保の128MiB予算」と「solve の性能チューニング用 chunk_size」を独立させる設計判断 (新しい設定フィールドか固定予算かの選択) が要り、3a の締めのタイミングで単独修正するにはスコープが合わないため。`docs/plans/rc-basics-03.md` T3 節「3a から持ち越した設計課題」に転記済み。 |
...
## サイクル 3a round 4 (`F-03-4-xxx`)

round 3 の修正自体の残債を埋める回 (BLOCKER 1 / MEDIUM 6 / INFO 6)。architecture
と security の2reviewerはこの回の着手前に「3a を締めてよい」と判定しており、
test reviewer が新設 `_format_target_count` の0テストを理由に BLOCKER を1件出して
締めを止めていた。本ラウンドの修正で3aを締める。

| ID | severity | 概要 |
|---|---|---|
| F-03-4-012 | BLOCKER | round3 で新設した `_format_target_count` (int→str 桁数上限を bit_length から概算した指数表記に落とす、F-03-3-021) にテストが1件も無く、素の `str` に戻す変異を注入しても ipc テストファイル全体 (43件) が1件も落ちなかった。`test_format_target_count_handles_huge_and_normal_totals` を追加し、(a) 巨大整数 (`10**5000`) で例外を投げない、(b) 指数表記になる、(c) 通常範囲では `str(total)` と一致する、の3点を固定した。 |
| F-03-4-001 | MEDIUM | 3a から3bへ送った項目 (F-03-1-005 / F-03-1-006 / `max_targets` の単位食い違い / `RowAlignment` 切り出し / F-03-3-007 / `diagnostics.ipc` の関数・モジュール名衝突) が、3bの実装者が読む正本 (`docs/plans/rc-basics-03.md` の T3 節) に1件も書かれていなかった。さらに `_capacity.py:101` の docstring が「呼び出し側が渡す前に読み取り専用にすることを3bの受け入れ条件とする」と、コードコメントで一方的に受け入れ条件を宣言していた。plan doc の T3 節冒頭に「3aから持ち越した設計課題」として6項目を列挙し、`CapacityProblem` に渡す `X` の read-only 化を T3 の受け入れ基準に1行として明記した。`_capacity.py` の docstring はこの受け入れ基準への相互参照に書き換えた。 |
| F-03-4-002 | MEDIUM | D-34 の rule は3節 (max_degrees既定値・絶対上限・max_variables絶対上限) を1件に詰めているが、guard_test (`test_max_degrees_bounds_the_psi_table_row_count`) は最初の1節しか固定していなかった (`_MAX_DEGREES` / `_MAX_VARIABLES_FOR_COUNT` を `10**9` に緩めても guard 単独は緑のまま)。guard_test を `test_out_of_range_config_raises` (パラメトライズ全体) に差し替え、cfg3/cfg6/cfg5 がそれぞれ(1)(2)(3)を独立に守ることを D-34 の rationale に明記した (D-33 と同じ形式)。 |
| F-03-4-003 | MEDIUM | `surrogate_capacities` が `_capacity.__all__` に載っていたが、モジュール外からの import は0件だった。この関数は「閾値を伴わないサロゲート容量」という、D-27 (『閾値は同じ関数で1箇所だけ計算する』) を回避する材料そのものを共有カーネルの公開面に置いていた。`_surrogate_capacities` へ private 化し `__all__` から外した (呼び出し元は `surrogate_threshold` のみ、ブロック化と分位点計算を分ける切り出しの意図は維持)。 |
| F-03-4-004 | MEDIUM | `surrogate_threshold` / `surrogate_capacities` の第2引数が `FloatArray` から `Iterable[FloatArray]` に広がったが、`np.ndarray` は `Iterable[np.ndarray]` を構造的に満たすため旧来の呼び方 (2次元配列を直接渡す) が mypy を素通りしていた (実測: mypy exit 0、実行時のみ分かりにくい `ValueError` に化ける)。`_reject_ndarray_base_blocks` を追加し、`base_blocks` に `ndarray` が直接渡された場合は専用の `TypeError` にした。`test_surrogate_threshold_rejects_bare_ndarray_base_blocks` で固定した。 |
| F-03-4-005 | MEDIUM | `docs/review-findings-03.md` の F-03-3-007 の送り理由「3bで `surrogate_capacities` 切り出しと同時にやると追加コストがほぼ無いため」が、その切り出しが round3 で既に完了していたため自己矛盾していた。送り理由を訂正し (分離には新しい設計判断が要るため3aの締めでは単独修正しない、という現状に即した理由に書き換え)、`docs/plans/rc-basics-03.md` の「3aから持ち越した設計課題」(F-03-4-001) にも転記した。 |
| F-03-4-007 | MEDIUM (security, CWE-400) | `_validate_combinatorial_bounds` が `max_delay_by_degree` の要素を「1以上」としか検査しておらず、値の**桁数**には上限が無かった。`math.comb` のコストは値ではなく桁数に対して約 `digits^1.6` で伸びるため、既定の `max_degrees<=32` / `max_variables<=20` の下でも `count_targets` を無防備に長時間 (`10**60000` で57.02秒) 走らせられた。`_MAX_DELAY_BIT_LENGTH=128` (bit) を追加し、`max_delay.bit_length()` を確保・計算より前に検査するようにした (値そのものは `str` 化しない)。D-34 の rule を3段から4段に更新し、`test_max_delay_bit_length_bounds_combinatorial_blowup_in_count_targets` と `test_out_of_range_config_raises` の新規ケース (`max_delay_by_degree=(1<<200,)`) で固定した。 |
| F-03-4-008 | INFO (security, CWE-209) | `_validate_config` の `heatmap_cells` 検査のメッセージが生の f-string で多倍長整数を `str` 化しており、round3 で塞いだ `enumerate_targets` 側より到達しやすい経路 (`ipc()` が最初に通る検査) に同種のクラッシュが残っていた。`heatmap_cells` / `max(max_delay_by_degree)` のメッセージを `_format_target_count` 経由にした (F-03-4-007 の桁数上限導入によりこの経路は通常到達不能になったが、二重に閉じた)。 |
| F-03-4-009 | INFO | `n_surrogates` / `n_surrogate_targets` は無制限の軸に見えるが、`n_surrogate_targets` は次数内の目標数で実効的に切り詰められ (`_surrogate_indices` の `min(n_selected, span)`)、`n_surrogates` は時間が線形・メモリは平坦であることの確認結果 (round3のsurrogate_capacities切り出し後もRSSは増えていない)。対応不要 (3bで絶対上限を置くかは3bの判断)。 |
| F-03-4-010 | INFO | 確保・計算量の全軸を洗い出し、`max_targets` を除く全軸が閉形式で確保前に縛られていることを確認した結果。`max_targets` は運用者が明示的に宣言する予算の終端であり、どんな予算方式でも必ず1本は絶対上限の無い軸が残るという性質のため、意図的に絶対上限を置かない。この方針を D-34 の rationale に明記し、次周以降で同じ指摘が再浮上しないようにした。 |
| F-03-4-011 | INFO | `_validate_combinatorial_bounds` の切り出しが `enumerate_targets` を自動的に保護すること、ipc() 本体の検査が弱くなっていないこと (42ケース中0件弱体化)、`_MAX_DEGREES=32` が3bの計画 (次数本数は既定4〜6本想定) と衝突しないこと、危険な API (eval/exec/subprocess/pickle/yaml.load 等) の新規混入が0件であることの確認結果。対応不要。 |
| F-03-4-013 | INFO | D-34 の guard_test が3段のうち1段しか単独では固定していないことの指摘 (F-03-4-002 と同一事象、testエージェントから独立に報告)。F-03-4-002 の対応 (guard_test差し替え・rationaleへの分担明記) で解消済み。 |
| F-03-4-006 | INFO | `surrogate_capacities` 追加時の責務分離 (ブロック化と分位点計算) が正しいこと、`_capacity.py` が665行で変更理由5つが同居する既存負債 (F-03-1-004、3bの別タスク候補) が悪化していないこと、`diagnostics/__init__.py` の関数 `ipc` 再エクスポートにより `import rc_basics_lab.diagnostics.ipc as m` がモジュールではなく関数を返す罠 (3bでモンキーパッチ時に注意が必要、plan docに転記済み) の確認結果。対応不要。 |

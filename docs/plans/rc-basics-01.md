...
- **想定所要**: L
- **実装時に決めたこと（仕様に無かった箇所。T5 と 02〜05 はこれに合わせる）**
  - **ESN の設定セクションは `esn_mackey_glass` / `esn_delay_parity` の2つ**にした（単一の `esn` ではない）。理由: T3 が課題ごとに別の既定値（MG は leak=0.3 / input_scale=0.5、パリティは leak=1.0 / input_scale=1.0）を定めており、1セクションにまとめると必ずどちらかの課題に不利な値を押し付けることになる。型はどちらも `ESNConfig` で、ローダは変更不要
  - **`*.length` は「生成される総行数」**であり、分割に使えるのは `length - max_start_offset - t0`。`config.yaml` では `length: 8200` とし、`8200 - 200 - 200 = 7800` 行を 0.5:0.15:0.35 = 3900/1170/2730 に切る（§3 の分割数と一致させるため）。dataclass 既定値の 8000 は変更していない
  - **分割窓の長さは split オフセットに依存しない**（`n_usable = T - max_start_offset - t0` で固定）。理由: オフセットで行数まで動くとレプリケート間で `n_train` が揺れ、受け入れ条件3 の平均±標準偏差の意味が濁る
  - **遅延パリティは入力を先に引き、先頭 `delay + n_bits - 1` 行の目標は「返さない ±1 履歴」から定義する**。理由: (a) 目標に NaN を混ぜない、(b) 同一シードなら `n_bits` / `delay` を変えても `u` が不変になり、`test_n_bits_and_delay_change_target` が「目標だけが変わった」ことを見られる。先読みに依存する数行は `t0 >= washout` により実験では評価されない
  - MG の初期履歴は `1.2 ± 0.1` の一様揺らぎ（task ストリーム）。理由: task シードを実配線し、未使用パラメータを作らない（D-06）。カオス系なのでバーンイン後に初期値の記憶は残らない
  - MG の RK4 中間段が要る `x(t-tau+h/2)` は履歴の**線形補間**。`tau / rk4_step` が整数でなければ `ValueError`（黙って丸めると設定した tau と積分した tau がずれる）
  - **手法は「候補 `FeatureSpec` の列」として表現する**（`Method.candidates`）。線形と ESN は候補1つ、遅延線は `n_lags_grid` ぶん。これにより学習・評価パスから手法名の分岐が消える（受け入れ条件1）。手法を列挙する場所は `build_methods` 1か所だけ
  - CSV の `n_lags` 列には `design.first_valid` を書く（遅延線では `n_lags`、線形・ESN では 0）。理由: 手法名で分岐せずに「特徴が何ステップ過去まで届くか」を1列で記録できる
  - **最終学習は train のみ**で行う（選択後に train+val で再学習しない）。理由: alpha と n_lags を選んだデータで係数まで引くと、選択の楽観バイアスが係数側にも入る。全手法に同じ規律が当たるため比較の公平性は保たれる
  - 候補が同点のときは**先に評価した候補（= `n_lags` が小さい方）**を残す（alpha は `select_alpha` の既存規約どおり大きい方を残す）。理由: 特徴数は少ない方が保守的
  - `TaskData` は `__post_init__` で形状・行数・有限性を検証する。理由: 設定 dataclass と違い生成結果の器であり、配線ミスを生成直後に落とせる場所がここしかない
  - `comparison.csv` の列順は `ResultRow` の宣言順（`CSV_COLUMNS`）を単一の真実とする。`meta.json` には `collect_meta` の内容に `wall_time_s` と `n_rows` を足す
  - `run.py --out` の既定は cwd 相対の `results`。図の追加（T5）はこの CLI に足す前提で、`run_experiment`（計算）と `report`（書き出し）を分けてある
  - テスト用の縮小設定 (`tests/test_experiment_fairness.py`) は **`max(n_lags_grid) > washout`** にしてある。理由: washout が全 `n_lags` を覆っていると、「手法ごとに `first_valid` を使う」誤実装でも行集合が偶然一致し、D-05 の guard が何も守らなくなる
  - **実測（5レプリケート・全30行、wall_time 0.75 秒）**: 遅延パリティは linear `1.0004 ± 0.0003` / delay_line `1.0007 ± 0.0007` / esn `0.0894 ± 0.0166`（符号正解率 0.500 / 0.522 / 1.000）で §3 の受け入れラインを満たす。MG は linear `0.1454 ± 0.0002` / delay_line `0.0008 ± 0.0000` / esn `0.0014 ± 0.0001` で、**MG では遅延線が ESN をわずかに上回る**（1ステップ先の MG は Δt=1.0 ではほぼ線形予測可能なため）。`n_lags` は常に格子上限の 16 が選ばれており、格子の端が効いている。記事では「MG だけ見ると遅延線で足りる。差が出るのはパリティ」という筋になる（格子の拡張は D-08 の探索予算の非対称性に触れるため、行うなら設計判断として別途相談する）。**→ この後にユーザー承認のうえ格子を拡張して再実行したため、上の数値は初版のものである。更新後の値と、当初の見立てがどう修正されたかは §4 追補を参照**

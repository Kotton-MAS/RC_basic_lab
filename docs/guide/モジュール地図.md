...
`src/rc_basics_lab/` の **134 モジュール**を層ごとに並べる。
...
| `config/_dump.py` | 設定 dataclass を「そのまま読み直せる」マッピングへ落とす (D-123) |
...
| `config/capacity03_ladder.py` | 実験 3-T (対照の梯子) の設定 dataclass (D-138 / D-139) |
...
| `tasks/protocol.py` | 課題の接合面 — **課題を足すときに満たすべき形** (D-123) |
| `tasks/registry.py` | 課題の生成口 — **生成関数を直接呼ぶ場所を1つにする** (D-123) |
| `tasks/synthetic.py` | 合成された異常系列の**生成パラメータ** (D-126) |
...
| `reservoir/axes.py` | 掃引軸 — **モデルごとに違う軸を、同じ掃引コードで振る** (D-124) |
...
| `reservoir/topology.py` | 結合構造 (トポロジ) をモデル本体から分離する層 (拡張性方針 §2-1) |
...
| `diagnostics/topology.py` | 結合行列 ``W`` を入力に取る診断 (D-122) |
...
| `experiment/catalog.py` | 実験の宣言を1箇所に置く (D-125) |
| `experiment/diagnostics_rows.py` | 診断のスカラを長形式で書き出す層 (§1 の「診断結果の出口」) |
| `experiment/esp.py` | 実験 2-A / 2-B / 2-C の配線層 —— ESN と診断層をつなぐ唯一の場所 |
| `experiment/esp_pipeline.py` | 1コマンドで 02 の7成果物を作る経路 (受け入れ条件7) |
| `experiment/esp_reservoir.py` | 1条件ぶんの ``ESNConfig`` を組む層 (D-138) |
| `experiment/esp_rows.py` | 02 の行の層 — ``esp_diagnostics.csv`` の行と、診断の長形式への畳み込み |
| `experiment/freerun.py` | 実験 4-A (教師強制の1ステップ先予測) と自走の入口の配線 (D-31 / D-44 / D-50) |
| `experiment/freerun_fit.py` | 教師強制で read-out を学習する層 (D-128) |
| `experiment/freerun_pipeline.py` | 1コマンドで 04 の成果物をそろえる経路 (受け入れ条件6) |
| `experiment/freerun_profile.py` | 自走の「配列」を長形式の行へ畳む層 (D-128) |
| `experiment/freerun_rows.py` | 実験04 の成果物の行 dataclass と、行からの要約 |
| `experiment/freerun_tasks.py` | 実験04 の課題の組み立てと、区間の検査 |
| `experiment/graph_diagnostics.py` | 結合行列そのものを測る診断を実験へ配線する層 (D-122 / D-132) |
| `experiment/horizon.py` | 実験 01' —— 自走 84 ステップ先の誤差 (NRMSE84) を測る (D-105) |
| `experiment/ladder_pipeline.py` | 梯子 (3-T / 3-Th) の実行と成果物の書き出し |
| `experiment/ladder_threshold.py` | 実験 3-Th: 閾値の選び方が梯子の結論を作っていないか (D-143) |
| `experiment/narma.py` | 実験 3-C —— 公平な対照での NARMA10 (D-31) |
| `experiment/narma_operating.py` | 実験 3-C'' —— NARMA10 の勝敗が**手法側の動作点**で変わることの実測 (D-144) |
...
| `experiment/rows_csv.py` | dataclass の行列を CSV に書く共通ヘルパ (D-126) |
...
| `experiment/stability_axes.py` | 4-C が振る軸と、条件からリザバー設定を組む場所 (D-124) |
| `experiment/stability_rows.py` | 04 の行の層 — ``stability.csv`` の行と、診断の長形式への畳み込み |
...
| `experiment/topology_ladder.py` | 実験 3-T: 交絡を1つずつ剥がす対照の梯子 (D-138) |
...
| `plotting/figures_ladder.py` | 実験 3-T の図 —— 対照の梯子が掃引のどこでも同じ結論を出すこと (D-145) |
...
| `plotting/figures_operating.py` | 実験 3-C'' の図 —— NARMA10 の勝敗が手法側の動作点で変わる (D-145) |

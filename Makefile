.PHONY: sync test cov lint fmt fmt-check type lock-check ci golden golden-update figures-01 figures-02 figures-03 figures-04 threshold-02 saturation-03 washout-02-unpadded pre-commit clean help
...
help:
	...
	@echo "  golden       - Byte-invariance check of artifacts (also part of test)"
...
# 成果物のバイト不変検査。リファクタリングの合否判定はこれで行う。
# 縮小設定 (tests/golden/configs/) で4実験を回し、CSV / PNG / meta.json の
# ダイジェストを tests/golden/manifest.json と突き合わせる (数秒)。
# test に含まれるので ci でも走るが、リファクタ中はこれだけを回すのが速い。
golden:

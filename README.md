...
### 実験01': 自走 84 ステップ先だとどうなるか（文献との照合）

01 本体が測っているのは**1ステップ先**だが、Mackey-Glass で広く引かれる文献値は
**NRMSE₈₄**（自走を 84 ステップ続けたときの、84 ステップ目だけの誤差）である。
予測長が違うと比較できないので、同じ量を足した（`horizon.csv` / `fig_horizon.png`）。

**1ステップ先を学習した同じ読み出しを、学習し直さずに自走させている。**

| | log₁₀(NRMSE₈₄) | 条件 |
|---|---|---|
| このリポジトリ | **−2.12 ± 0.29** | `N = 200`、出力フィードバック学習なし、5 レプリケート |
| Jaeger & Haas 2004 の ESN | **−4.20** | `N = 1000`、出力フィードバックつき自走、100 試行 |
| 同論文が挙げる従来手法 | **−1.35** | 同論文が「700 倍悪い」とする水準 |

- **従来手法の水準は上回るが、文献の ESN には 2.1 桁届かない。**
  リザバーが 5 分の1（200 対 1000）で、出力フィードバックつきの学習もしていないので、
  この差は条件差と整合する。**同じ土俵に立ったうえで差を示すこと**が目的である
- 1ステップ先では ESN と遅延線がほぼ並ぶ（上の表）のに、84 ステップ先まで自走させると
  差が出る。**予測長を変えると結論が変わる**という、この連載で繰り返し出てくる形
...
| `horizon.csv` | 5行。01' の自走 84 ステップ先の誤差（ESN × 5レプリケート） |
| `narma10_taps.csv` | 48行。3-C' のタップ数掃引 (8点 × 2手法 × 3レプリケート) |
...
- **ただしそれは動作点が先行と違うからである** (実験 3-C' / D-95)。
  3-C の動作点は `k / n_train <= 0.01` で、先行は `k / n ≈ 0.9` だった。
  タップ数を振ると (`fig_narma10_taps.png` / `narma10_taps.csv`)、
  正則化の有無は小さい k では区別できず、`k / n_train` が 1 に近づいたところで
  **OLS だけが壊れる**:

| k / n_train | 0.003 | 0.026 | 0.476 | 0.786 | **0.943** |
|---|---|---|---|---|---|
| OLS / リッジ の NMSE 比 | 1.00 | 1.02 | 1.40 | 1.60 | **3.33** |

  つまり先行の対照設計の穴は「正則化なし」単独ではなく
  **「正則化なし かつ k ≈ n_train」** の合わせ技だった、と実測で言える

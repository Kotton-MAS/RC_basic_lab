...
**測っていない範囲を明記する** (D-149)。上の実装差は GitHub Actions の macOS
runner と開発機の比較で、**どちらも BLAS は Accelerate である**
(``numpy.__config__`` で確認)。したがってこの 1.5e-3 は *Accelerate 同士*の差
であって、OpenBLAS / MKL との差ではない。**別の BLAS で 5e-3 を超えるかは
未測定**である。超える環境が見つかったら、許容差を上げる前に

1. その環境の実装差を測って上の表に足す
2. 上限側 (最小の構造変更 7.6e-3) との余裕が残るかを確かめる

の順で判断すること —— 余裕は 5 倍しかないので、実装差が 1.5e-3 の 3 倍を
超えたら、許容差ではなく**照合の仕方**を変える側 (``nmse`` を恒等式検査へ
回したのと同じ手) を先に検討する。
"""

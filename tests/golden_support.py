"""成果物のバイト不変検査 (ゴールデン) の共通機構.

リファクタリングの合否を「**成果物が 1 バイトも変わらないか**」で判定するための
道具である。行の値を検証するテストは既に厚いが、それらは構造を動かすと一緒に
書き換わってしまうため、「テストの修正」と「リファクタの失敗」が区別できない。
ここはコードを読まず、出力だけを見る。

本番設定 (``experiments/*/config.yaml``) は 03 だけで数十分かかるので、**同じ
``run_and_report_*`` 経路**を通しつつ系列長・格子・レプリケート数だけを縮めた
設定を ``tests/golden/configs/`` に置き、その出力のダイジェストを
``tests/golden/manifest.json`` に固定する。縮小設定から科学的な結論は出ない。
ここで見るのは同一性だけである。

"""再生成した成果物のバイト不変検査 (ゴールデン).

リファクタリングの合否判定はここ 1 本で行う。**コードを読まず、出力だけを見る**
のが役割で、行の値を検証する他のテスト群 (構造を動かすと一緒に書き換わる) とは
目的が違う。

``tests/test_artifact_invariance.py`` (D-74) が「コミット済みの ``results/`` が
変わっていないこと」を測るのに対し、ここは「**同じ設定から作り直した**成果物が
変わっていないこと」を数秒で測る。機構・除外規則・実験05を対象外にする理由は
``tests/golden_support.py`` の docstring にある。基準値の更新は ``make golden-update``。

...
多数の目標 ``z_k`` を線形読み出しでどれだけ説明できるか」を測る同一の量で、
違いは目標の作り方だけである。回帰と容量の計算はここに1本だけ置き、
``memory_capacity`` / ``ipc`` は目標の生成だけを持つ。

このモジュールが守る不変条件:

1. ``Phi = [1, X]`` を実体化せず、Gram をブロック分解
   ``[[T_eff, sum(X,0)], [sum(X,0).T, X.T @ X]]`` で構築時に1回だけ作る
   (D-26 / F-03-1-013)。
2. 容量は Gram 量だけで閉じる。残差は
   ``z.T z - 2 w.T rhs + w.T G w`` で評価し、予測 ``Phi @ W`` を作らない
   (D-26)。
3. サロゲートも通常の目標と同じ経路・同じチャンク化を通る (D-27 /
   F-03-1-012)。生成順序は ``chunk_size`` に依存しない。
4. 基底は宣言された入力分布に対して正規直交で、分布と基底は
   ``InputMeasure`` 1値としてのみ渡せる (D-28)。
5. 行合わせの担い手は ``RowAlignment`` 1つ (D-24)。
6. チャンク幅は性能軸 (``solve_width``) と確保軸 (``block_width``) に分け、
   どちらも ``bounded_chunk_size`` へ委譲する (D-33)。

``readout.ridge.fit_ridge_from_gram`` への依存は D-23 で許可されている。

非公開モジュール (先頭が ``_``) なので ``diagnostics/__init__.py`` からは
公開せず、``tests/test_public_api_reexport.py`` の対象外である。
"""
...
"""``(input_distribution, basis)`` の対応表 (D-28)。ここに無い組は ``ValueError``。"""
...
class InputMeasure:
    ...
    分布と基底は対でのみ意味を持つため、片方だけを渡す呼び方が書けないよう
    1つの値にしてある (F-03-1-006)。組の検証は**構築時点**で行う。設定層
    (``IpcConfig``) は YAML と ``meta.json`` の面を変えないため2つの文字列
    フィールドのまま保ち、``ipc()`` が入口で1度だけ畳む。
...
@dataclass(frozen=True, slots=True)
class RowAlignment:
    """MC / IPC が共有する行合わせ (D-24)。``t0`` と ``n_samples`` だけを持つ。

    (i) 基準点 ``t0 = max(washout, その診断の最大遅延)`` の算出、(ii)
    ``t0 >= T`` の拒否、(iii) 遅延窓の切り出しをここ1か所に置く
    (F-03-1-001)。**状態行列にも Gram にも触れない**ので、行合わせだけを
    検査するテストがダミーの状態行列を作らずに書ける
    (``test_row_alignment_needs_no_state_matrix``)。

    チャンク幅の2軸 (D-33) もここに置き、``bounded_chunk_size`` へ委譲する。
    ...
    def lagged(self, series: FloatArray, delay: int) -> FloatArray:
        ...
        ``series[t0 - delay : t0 - delay + n_samples]`` を返す。「遅延 k の
        目標は状態と同じ行集合 (``t0`` 始まり) に対応する」という D-24 の実体は
        この1本の窓計算だけが持つ (F-03-1-001)。
    ...
    def solve_width(self, configured: int) -> int:
        ...
        ``cfg.chunk_size`` を上限とし、1チャンクが ``_MAX_CHUNK_BYTES`` を
        超えるときだけ下げる (結果は変わらない)。呼び出し側は
        ``cfg.chunk_size`` を直接使わずこのメソッドを経由し、実効値を
        ``params['chunk_size_effective']`` に記録する (F-03-2-001 /
        F-03-2-009)。
        """
    ...
    def block_width(self, n_columns: int) -> int:
        ...
        ``cfg.chunk_size`` を**読まない**。上限は ``n_columns`` 自身と
        128 MiB 予算だけで決まる。
...
class CapacityProblem:
    ...
    設計行列 ``Phi = [1, X]`` は実体化せず、Gram をブロック分解で作る
    (D-26 / F-03-1-013)。``eq=False`` なのは numpy 配列を持つ dataclass の
    ``__eq__`` が ``ValueError`` になるため (同一性の比較には使わない)。

    **契約** (F-03-2-003): ``x`` は呼び出し側の ``X`` の**ビュー**、``gram``
    は構築時点の**スナップショット**である。``from_states`` の後で ``X`` を
    書き換えると両者が例外もなく desync するため、渡した後の ``X`` は不変と
    して扱うこと。``x`` 自身は ``writeable=False`` で保持するので
    ``problem.x[...] = ...`` は ``ValueError`` になる (F-03-3-006) が、元の
    ``X`` への書き込みは塞げない (F-03-4-001)。
    ...
    def from_states(cls, X: FloatArray, *, rows: RowAlignment) -> CapacityProblem:
        ...
        ``Phi`` は作らない。バイアス列の寄与は ``X`` の縮約だけで閉じるため、
        ``X`` に触れるのは ``X.T @ X`` の1回で済む (D-26)。
...
def capacity_of_targets(
    ...
    容量は ``C_k = 1 - ||z_k - Phi w_k||^2 / ||z_k||^2`` (Dambre 2012)。残差は
    予測を実体化せず Gram 量だけで展開して評価する (D-26 / F-03-1-013)::

        ||z - Phi w||^2 = z.T z - 2 w.T (Phi.T z) + w.T (Phi.T Phi) w

    ``rhs = Phi.T @ Z_chunk`` はバイアス行 ``sum(Z_chunk, 0)`` と ``X`` 行
    ``X.T @ Z_chunk`` にブロック分解するため、``X`` に触るのは1回だけ。
...
def orthonormal_basis(
    ...
    返す列は、宣言された入力分布のもとで平均 0・分散 1・異なる次数と直交に
    なる。正規化は入力の**実測**の平均と標準偏差で行い、一様分布では半値幅
    ``a = sqrt(3) * sigma_u`` (D-17) で割ってから Legendre に入れる。

    次数 1 は分布によらず ``(u - mean) / sigma`` に一致する。MC が入力分布を
    設定項目に持たず ``UNIFORM_LEGENDRE`` 固定で足りるのはこのため。
    ...
    Args:
        ...
        measure: 入力測度 ``InputMeasure(distribution, basis)`` (D-28)。
            **既定値を持たない** (F-03-1-006)。未対応の組は ``InputMeasure``
            の構築時点で落ちるため、ここには組の検査を置かない。
...
def input_series(u: FloatArray | None, *, diagnostic: str) -> FloatArray:
    ...
    MC と IPC が複製していた入力検証をここ1本にまとめたもの (F-03-1-020)。
...
generator + 呼び出し側ループの構造上、一時的に2チャンク分が同時に生きる。
128 MiB は「2チャンク同時生存 + 状態行列 X + 一時配列」を足しても 4GB 予算に
収まるよう選んだ値 (根拠の実測は F-03-1-012 / F-03-1-013)。
"""
...
def bounded_chunk_size(configured: int, n_samples: int) -> int:
    ...
    ``chunk_size`` は結果を変えない性能パラメータなので、下げても数値は
    1ビットも変わらない。``configured`` より大きくはしない。本番規模では
    ``configured`` の値によらず ``_MAX_CHUNK_BYTES`` 相当まで切り詰める
    (D-33)。**128 MiB 予算を持つ唯一の場所**であり、性能軸
...
def _iter_surrogate_chunks(
    ...
    生成順序は ``index = 0 .. n_base * n_surrogates - 1`` の単調増加で固定し、
    ``chunk_size`` がどこで区切っても ``rng`` の消費順序が変わらないように
    する。結果は ``chunk_size`` に依存しない
...
def _reject_ndarray_base_blocks(base_blocks: Iterable[FloatArray]) -> None:
    """``base_blocks`` に ``ndarray`` が直接渡された誤用を ``TypeError`` にする。

    ``np.ndarray`` はイテレートすると行を返すため ``Iterable[FloatArray]`` を
    構造的に満たし、旧来の呼び方 (2次元配列を直接渡す) が mypy を素通りする。
    型で防げないので境界で落とす (F-03-4-004)。
    """
...
def _surrogate_capacities(
    ...
    *,
    ...
    ``base_blocks`` を **Iterable of block** にすることで、ブロック化 (呼び
    出し側が 128MiB 予算にどう分けるか) と分位点計算 (この共有カーネル1本)
    を分離する (F-03-3-002)。1ブロックしか無い呼び出し側 (MC) は ``[base]``
    を渡す。

    非公開なのは D-27 (閾値は同じ関数で1箇所だけ計算する) を回避する材料を
    公開面に置かないため (F-03-4-003)。
...
def surrogate_threshold(
    ...
    *,
    ...
    各列を時間方向にシャッフルした系列を ``n_surrogates`` 本ずつ作り、
    **通常の目標とまったく同じ経路** (``capacity_of_targets``) に流して容量を
    測る。閾値の分位点はこの関数**だけ**が計算する (D-27 / F-03-3-002)。
    シャッフル列は ``chunk_size`` 列ずつその場で生成しては捨てる
    (F-03-1-012)。
...
def chi2_threshold(*, n_units: int, n_samples: int, quantile: float) -> float:
    ...
    1つ (``THRESHOLD_CHI2``) が使う (F-03-1-003)。MC は次数1しか評価せず
    サロゲートで足りるため呼ばない。
    """
...
def capacity_of_chunks(
    ...
    引数が **iterable of chunk** なのは D-26 のため: 呼び出し側は
    ``(T_eff, chunk_size)`` を1枚ずつ作って渡し、畳んだら捨てられる。全目標を
    ``(T_eff, K)`` として実体化しない。列ごとの容量は独立に決まるので、
    チャンクの切り方は結果を変えない
    (``test_chunk_size_does_not_change_results``)。

...
1ステップ進める写像を ``StateUpdater`` プロトコルで受け取るので、ESN でも、
外部シミュレータでも、実素子の測定ループでも、同じ ``free_run`` がそのまま
動く (要件書 受け入れ条件7: 移植性の担保)。``diagnostics`` が
``StatePropagator`` で ``reservoir`` から独立しているのと同じ形である。

自走は**教師強制で学習した read-out 係数をそのまま使う** (D-44)。``free_run``
は係数を引数で受け取るだけで、学習の入口 (``fit_ridge`` / ``select_alpha``) を
一切呼ばない —— 自走のたびに学習し直すと「自走が上手くいくか」ではなく
「自走区間に当てはめ直せるか」を測ることになり、実験の意味が変わる
(仕様 §5 禁止する構造1・2)。返り値の ``coefficients`` は**渡された配列その
もの** (同一オブジェクト) なので、内部で学習し直していないことを呼び出し側が
同一性で検査できる。

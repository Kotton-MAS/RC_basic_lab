...
**3.2 から締めた** (2026-08-31)。3.2 は「banner の 3:1 + 余裕」という体裁だけの
根拠だったが、読み手側から決め直すと別の数字になる —— Zenn の本文幅は概ね
700px なので、比 2.9 の図 (``fig_ipc_profile`` は 4 パネル横一列だった) は
1 パネルあたり 165px しか残らず、記事の中で読めない。

2.3 は**全 22 枚を 2 段化・高さ調整したあとの実測の最大値** (2.18、
``fig_threshold_tradeoff``) に丸め誤差ぶんを足したもの。これを超えるなら
**パネルを2段に折る** (実例: ``fig_ipc_profile`` を 1x4 -> 2x2 にして 2.88 -> 1.26)。
"""
...
横に折るか系列を減らす。**2 段化は下限を割る方向にも効く** —— 段あたりの高さを
そのままに 1x4 を 2x2 へ折ると ``fig_ipc_profile`` が 0.86 : 1 になった
(段あたりの高さをヒートマップの縦横比に合わせて 1.26 : 1 に直した)。
"""

...
図の外枠 (rcParams の一時適用 / ``Figure`` の生成 / 保存) は
``plotting/_canvas.py`` が持つ。ラベルは必ず ``style.label(ja, en)`` を通す
(D-10)。**ギリシャ文字は書かない** (RUF001/RUF002。``rho`` / ``sigma_u`` と綴る)。

**配列ではなく長形式の行を読む** (D-38): 遅延プロファイルとヒートマップは
``CapacityProfileRow`` から復元する。長形式には**容量が厳密に正のセルだけ**が
在るので、格子に戻すときは**欠けているセルを 0 で埋める** (しきい値を超え
なかったセルの容量は 0)。診断はここでは一切走らせない。
"""

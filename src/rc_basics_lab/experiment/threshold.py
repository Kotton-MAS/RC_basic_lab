...
実測 (計測ラッパで ``simulate_condition`` / ``esp_convergence`` の呼び出し回数と
累積時間を集計): 総実行時間 63.30秒のうち ``simulate_condition`` (336回) が
53.65秒 (84.8%)、``esp_convergence`` の9倍呼び出し (3024回) は 9.62秒 (15.2%)
に過ぎない。距離行列の再計算コストは全体の「約半分」ではなく「約15%」であり、
9回を1回に減らしても短縮できるのは約8.5秒 (全体の約13.5%) にとどまる。
削減幅が小さい以上、``esp_convergence`` を本物の診断関数のまま9回呼ぶ
(判定ロジックを別実装で近似しない) という現状の判断はなおさら妥当である。
この 63.30秒 は上記の計測ラッパのオーバーヘッドを含むため、README.md /
``docs/design.md`` にある ``make threshold-02`` 実測の 60.70秒 とは一致しない
(どちらも同じコマンドの実行時間だが、計測経路が異なる)。

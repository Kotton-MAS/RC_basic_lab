...
Lorenz の x は i.i.d. ではなく強く時間相関した決定的系列なので、IPC が前提と
する直交多項式基底の仮定が成り立たず、``ipc_total`` は保存則 (<= N) を超える。
04 は 03 の接ぎ目をそのまま使う (仕様 §5 禁止する構造4: 条件ごとに ESN を
2回作らない) ことを優先しており、**``ipc_total`` の絶対値は 03 の掃引と
比較できない**。条件間の相対比較と ``mc_total`` (N で抑えられている) の方を
読むこと。
"""

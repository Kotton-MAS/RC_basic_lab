...
def normalized_error_curve(truth: FloatArray, prediction: FloatArray) -> FloatArray:
    ...
    ``prediction`` の ``nan`` (打ち切られた後の行) は誤差も ``nan`` になり、
    ``valid_time_from_errors`` がこれを「閾値を超えた」と扱う。
...
def point_set_distance(left: FloatArray, right: FloatArray) -> float:
    ...
    2次元ヒストグラムの全変動距離を採らなかったのは実測による
    (``docs/design.md`` §12)。ビン数も分位点数も持たないので、確保軸7
    (「ビン数を独立の軸にしない」) は軸が存在しない形で満たす。確保量は
    ``_POINT_CHUNK`` で両側をブロック化して抑える。
...
def shuffled_surrogate(
    ...
    行 (時刻) を並べ替えるので周辺分布は変わらず**時間構造だけ**が消える。
    「自走がアトラクタを再現している」という主張は、この対照より有意に近い
    ことで初めて成立する (D-46)。

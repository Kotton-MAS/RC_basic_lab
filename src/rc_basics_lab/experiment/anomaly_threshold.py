...
- ``calibrate_threshold(calibration_scores, target_false_alarm_rate)`` は
  **ラベルを1つも受け取らない**。運用閾値は較正区間のスコア分位点だけで
  決まり、テスト区間では固定される。テスト側で閾値を選ぶと誰でも高性能に
  見えるが、それは「良い結果」として出るためレビューでは止まらない ——
  引数から外せば、混入が型検査で書けなくなる。
- テスト側最適化は ``best_test_f1`` という**別の関数**で、名前も戻り値も
  「参考値」であることを言っている。結果は ``f1_test_optimal`` という
  **別列**にのみ出る (D-56)。

``best_test_f1`` は掃引の格子ではなく **PR 曲線の全点**の最大を返す。
``metrics_detection.precision_recall_curve`` の点は「相異なるスコアで切った
ときの (P, R)」の全体なので、どんな閾値の (P, R) も必ずこの点列のどれかに
一致する —— したがって ``f1_test_optimal >= f1_calibrated`` が**定義から**
成り立つ (仕様 §4 T3 の受け入れ基準5)。格子で近似すると、格子の粗さ次第で
この不等号が破れる行が出る。
